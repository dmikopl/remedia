...
.PHONY: help build up down restart shell composer install update console cs cs-fix phpstan test test-coverage logs cache-clear permissions migrate bootstrap
...
permissions:
	$(DOCKER_COMPOSE) exec -T -u root php sh -c "mkdir -p /app/var/cache /app/var/log /app/var/data && chown -R app:app /app/var /app/vendor"

bootstrap:
	$(DOCKER_COMPOSE) build
	$(DOCKER_COMPOSE) down --remove-orphans
	$(DOCKER_COMPOSE) up -d
	$(DOCKER_COMPOSE) exec -T -u root php sh -c "mkdir -p /app/var/cache /app/var/log /app/var/data /app/vendor && chown -R app:app /app"
